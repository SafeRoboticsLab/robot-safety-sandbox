...
Fixes the split-env spawn bug: split_v2 removes the joint-reset event, so we set
the FULL standing pose (root at default height, 1.5 m back, + default joints).

  python tools_e040/gap_filter_test.py --safety runs_e025_ref/avoid_w30/final_model.zip \
      --label avoid --out tools_e040/gap_avoid.mp4
  python tools_e040/gap_filter_test.py --safety runs_e040b/go2_gap_split2_ra_w30/final_model.zip \
      --label RA    --out tools_e040/gap_ra.mp4
"""
...
def reset_standing_full(env, env_ids, asset_cfg=SceneEntityCfg("robot")):
  ...
  joints (split_v2 dropped the joint reset -> must set it or the robot folds)."""
  if env_ids is None:
