"""E040 value-ordering probe (local, corrected codebase).

At near-edge decision states across a momentum sweep, does each critic VALUE
crossing? The reach term should make RA's V(spawn) RISE with feasibility (it
banks the reachable far target); avoid's viability value stays ~flat-high
(stopping is always safe -> indifferent to crossing). Under the g-anchor BUG,
RA collapses toward that same flat-high, over-certifying non-crossing states at
~g. The tell for the fix: corrected-RA's V slope vs momentum should be POSITIVE
and separate from both avoid and buggy-RA.

Three critics, same spawn distribution:
  avoid     = E025 SafetyPPO   final   (runs_e025_ref/avoid)
  buggy-RA  = E025 ReachAvoidPPO final  (runs_e025_ref/buggy_ra, g-anchor)
  corr-RA   = the corrected run's checkpoint (--ra-model ...)

Usage:
  python tools_e040/probe_value_ordering.py --ra-model runs_e040/checkpoints/model_XXXX_steps.zip
"""
import argparse
import os
import sys
...
from robot_safety_sandbox.envs.go2_gap import split_v2 as S
...
p.add_argument("--task", default="go2_gap_split2_ra",
               help="split task (sets gap width): _ra / _ra_w20 / _ra_w30")
p.add_argument("--avoid-model", default="runs_e025_ref/avoid",
               help="E025 avoid baseline run dir for this width")
p.add_argument("--buggy-model", default="runs_e025_ref/buggy_ra",
               help="E025 buggy-RA baseline run dir for this width")
...
TW = [("avoid   ", SafetyPPO, dict(run_dir=args.avoid_model)),
      ("buggy-RA", ReachAvoidPPO, dict(run_dir=args.buggy_model)),
      ("corr-RA ", ReachAvoidPPO, dict(zip_path=args.ra_model))]
