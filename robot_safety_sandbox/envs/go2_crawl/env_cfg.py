...
def reset_crawl_trajectory(env, env_ids, asset_cfg=SceneEntityCfg("robot")):
  ...
  if len(env_ids) == 0:
    return
  from robot_safety_sandbox.tasks.go2_crawl import _PIT_BEHIND  # lazy: avoid import cycle
  _ensure_crawl_buffers(env)
  asset = env.scene[asset_cfg.name]
  device = env.device
  n = int(len(env_ids))
...
def caught_by_pit(env, env_ids=None):
  ...
  from robot_safety_sandbox.tasks.go2_crawl import _pit_edge  # lazy: avoid import cycle
  edge = _pit_edge(env)
  if edge is None:
